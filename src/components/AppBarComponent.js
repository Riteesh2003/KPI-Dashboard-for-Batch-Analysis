...
import { AppBar, Typography, Button, Box, TextField } from "@mui/material";
...
const AppBarComponent = () => {
  ...
  return (
    <ThemeProvider theme={theme}>
      <CssBaseline />
      <AppBar position="static" sx={{ padding: "15px 16px", display: "flex" }}>
        {/* Left Side: Title */}
        <Typography variant="h6">
          Batch Dashboard &gt; KPI indicators
        </Typography>
      ...
      {/* Controls Box with Run Button, Product Info, Theme Toggle, and Logout */}
      <Box
        id="controlBox"
        sx={{
          width: "100%",
      ...
      >
        ...
        >
          ...
          {/* Autocomplete (Search Reference Batch) */}
          <Autocomplete
            options={referenceBatches.filter((batch) =>
              batch.toLowerCase().includes(searchValue.toLowerCase())
            )}
        ...
        {/* Right Side: Theme Toggle and Logout (Pushed to Right) */}
        <Box
          sx={{
            marginLeft: "auto",
            display: "flex",
